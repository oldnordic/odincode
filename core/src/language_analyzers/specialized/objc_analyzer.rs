//! Objective-C-specific language analyzer
...
impl ObjCAnalyzer {
    ...
    fn analyze_objc_issues(
        ...
        match node.kind() {
            ...
            "interface_declaration" | "implementation_declaration" => {
                if let Some(name_node) = node.child_by_field_name("name") {
                    ...
                    if !name.chars().next().unwrap_or('_').is_uppercase() {
                        issues.push(CodeIssue {
                            ...
                            description: format!(
                                "Objective-C class name '{}' should start with uppercase letter",
                                name
                            ),
        ...
        for child in node.children(&mut cursor) {
            self.analyze_objc_issues(child, file_content, issues)?;
        }

        Ok(())
    }

    fn generate_objc_suggestions(
        &self,
        node: Node,
        file_content: &str,
                ...
                if let Some(name_node) = node.child_by_field_name("name") {
                    ...
                    if !name.chars().next().unwrap_or('_').is_uppercase() {
                        suggestions.push(CodeSuggestion::new_complete(
                            ...
                            "Objective-C class names should follow PascalCase convention",
                            Severity::Warning,
                            None,
        ...
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
            self.generate_objc_suggestions(child, file_content, suggestions)?;
        }
...
impl LanguageAnalyzer for ObjCAnalyzer {
    fn analyze_issues(&self, tree: &Tree, file_content: &str) -> Result<Vec<CodeIssue>> {
        let mut issues = Vec::new();
        self.analyze_objc_issues(tree.root_node(), file_content, &mut issues)?;
        Ok(issues)
    }

    fn generate_suggestions(&self, tree: &Tree, file_content: &str) -> Result<Vec<CodeSuggestion>> {
        let mut suggestions = Vec::new();
        self.generate_objc_suggestions(tree.root_node(), file_content, &mut suggestions)?;
        Ok(suggestions)
    }
}
