//! Dart-specific language analyzer
...
impl DartAnalyzer {
    ...
    fn analyze_dart_issues(
                ...
                if let Some(name_node) = node.child_by_field_name("name") {
                    ...
                    if !name.chars().next().unwrap_or('_').is_uppercase() {
                        issues.push(CodeIssue {
                            ...
                            description: format!(
                                "Dart class name '{}' should start with uppercase letter",
                                name
                            ),
        ...
        for child in node.children(&mut cursor) {
            self.analyze_dart_issues(child, file_content, issues)?;
        }

        Ok(())
    }

    fn generate_dart_suggestions(
        &self,
        node: Node,
        file_content: &str,
        ...
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
            self.generate_dart_suggestions(child, file_content, suggestions)?;
        }
...
impl LanguageAnalyzer for DartAnalyzer {
    fn analyze_issues(&self, tree: &Tree, file_content: &str) -> Result<Vec<CodeIssue>> {
        ...
        self.analyze_dart_issues(tree.root_node(), file_content, &mut issues)?;
        Ok(issues)
    }

    fn generate_suggestions(&self, tree: &Tree, file_content: &str) -> Result<Vec<CodeSuggestion>> {
        let mut suggestions = Vec::new();
        self.generate_dart_suggestions(tree.root_node(), file_content, &mut suggestions)?;
        Ok(suggestions)
    }
}
