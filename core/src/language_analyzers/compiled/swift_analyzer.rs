//! Swift-specific language analyzer
...
impl SwiftAnalyzer {
    ...
    fn analyze_swift_issues(
        ...
        for child in node.children(&mut cursor) {
            self.analyze_swift_issues(child, file_content, issues)?;
        }

        Ok(())
    }

    fn generate_swift_suggestions(
        &self,
        node: Node,
        file_content: &str,
        ...
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
            self.generate_swift_suggestions(child, file_content, suggestions)?;
        }
...
impl LanguageAnalyzer for SwiftAnalyzer {
    fn analyze_issues(&self, tree: &Tree, file_content: &str) -> Result<Vec<CodeIssue>> {
        ...
        self.analyze_swift_issues(tree.root_node(), file_content, &mut issues)?;
        Ok(issues)
    }

    fn generate_suggestions(&self, tree: &Tree, file_content: &str) -> Result<Vec<CodeSuggestion>> {
        let mut suggestions = Vec::new();
        self.generate_swift_suggestions(tree.root_node(), file_content, &mut suggestions)?;
        Ok(suggestions)
    }
}
